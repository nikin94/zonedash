...
/**
 * The persistent app header, above the tab navigator: the title on the left and
 * the central-unit status pill on the right. Everything else moved to the footer
 * tabs — this header is now just identity + link status. A pill tap connects
 * while disconnected; while connected it opens a dropdown with Re-pair (only
 * with a layout to rebind) and Disconnect (behind a confirm).
 */
export const AppHeader = ({
  onRepair,
}: {
  /** Rebind the layout: reset the drill surface to pairing and go to Drill.
